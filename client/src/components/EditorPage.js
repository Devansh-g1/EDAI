...
function EditorPage() {
  ...
  const runCode = async () => {
    setIsCompiling(true);
    try {
      const response = await axios.post("http://localhost:4001/compile", {
        code: codeRef.current,
  ...
  const getSuggestion = async () => {
    try {
      const response = await axios.post("http://localhost:4001/suggest", {
        code: codeRef.current,
        language: selectedLanguage,
      });
      console.log("Suggestion:", response.data.suggestion);
      // Add logic to apply the suggestion to your editor
      ...
      <div className="row flex-grow-1">
        ...
        <div className="col-md-2 bg-dark text-light d-flex flex-column">
          ...
          <div className="mt-auto mb-3">
            ...
            <button className="btn btn-info w-100" onClick={getSuggestion}>
              AI SUGGESTED
            </button>
          </div>
        </div>
